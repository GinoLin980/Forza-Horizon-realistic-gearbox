...
def append_data():
    global gas_list, brake_list, gear_list, rpm_max_list, shift_status_list, jump_gears_list
    gas_list.append(gas)
    brake_list.append(brake)
    gear_list.append(gear)
    rpm_max_list.append((rpm-idleRPM) / max_shift_rpm)
    shift_status_list.append(shift_status)
    jump_gears_list.append(kickdown if jump_recorded == False else False)

def csv_update(gas: list, brake: list, rpm_max: list, gear: list, shift_status: list, jump_gears: list):
    
    new_row = pd.DataFrame({'rpm/max': rpm_max, 'gas': gas, 'brake': brake, 'gear': gear, 'shift_status': shift_status, 'jump_gears': jump_gears})

    # Append the row to the CSV file
    new_row.to_csv('data/data.csv', index=False, mode='a', header=False)

count = 0

def main():
    global rt, addr, shift_status, count
...
if __name__ == "__main__":
    main()
    csv_update(gas=gas_list, brake=brake_list, rpm_max=rpm_max_list, gear=gear_list, shift_status=shift_status_list, jump_gears=jump_gears_list)